...
export function manifestPathFor(bundleDir) {
    return join(bundleDir, MOD_MANIFEST_FILENAME);
}
export function loaderPathFor(bundleDir) {
...
export function digestEntries(bundleDir) {
    const entries = {};
...
function isDigest(value) {
    if (value === null || typeof value !== "object")
        return false;
    const raw = value;
    return (typeof raw.bytes === "number"
...
export function parseManifest(text) {
    ...
    const vencord = (raw.vencord ?? null);
    if (vencord === null
    ...
    const rawEntries = (raw.entries ?? null);
 ...
 */
export function inspectBundleDir(bundleDir) {
    ...
    if (manifest === null)
        return { manifest: null, loaderPath, problems: [problem ?? "unknown"] };
    const problems = [];
    ...
    if (existsSync(stampedPath)) {
        ...
        if (!carriesStamp) {
            problems.push(`${STAMPED_ENTRY_NAME} does not contain build id ${manifest.buildId}, so ${MOD_MANIFEST_FILENAME} `
