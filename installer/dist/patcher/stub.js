...
export function stubIndexSource(loaderPath) {
    return `require(${JSON.stringify(loaderPath)})`;
}
export function buildStubAsar(loaderPath) {
    return buildAsar([
...
export function parseRequirePath(indexSource) {
 ...
 */
export function readStub(asarPath) {
    const dir = readAsarDirectory(asarPath);
    if (!dir.ok)
        return dir;
    const names = new Set(dir.value.names);
    const isStubShape = names.size === 2 && names.has(STUB_INDEX_NAME) && names.has(STUB_PACKAGE_NAME);
    if (!isStubShape)
        return ok(null);
    const totalSize = dir.value.files.reduce((sum, f) => sum + f.size, 0);
    if (totalSize > MAX_STUB_BYTES)
        return ok(null);
...
export function readIsOriginalAsar(asarPath) {
