 ...
 */
export function beaconDirFor(platform = process.platform, env = process.env, home = homedir()) {
    return productDirFor(platform, env, home);
}
export function beaconPathFor(platform = process.platform, env = process.env, home = homedir()) {
...
function isErrorCode(value) {
    return typeof value === "string" && BEACON_ERROR_CODES.includes(value);
}
/**
...
export function toBuildId(value) {
 ...
 */
export function validateBeacon(parsed, path) {
    if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
        return err("BEACON_MALFORMED", "The status file is not a JSON object.", { path });
    }
    const raw = parsed;
    if (raw.product !== "subline") {
        return err("BEACON_MALFORMED", "The status file was not written by Subline.", { path });
    }
    if (raw.format !== SUPPORTED_BEACON_FORMAT) {
        ...
        // confirmation.
        return err("BEACON_FORMAT_UNSUPPORTED", `The status file is format ${String(raw.format)}, and this installer understands ${SUPPORTED_BEACON_FORMAT}.`, { path });
    }
    ...
    if (loadedAt === null) {
        ...
        return err("BEACON_MALFORMED", "The status file has no usable load timestamp, so it cannot be matched to this installation.", { path });
    }
    const rawCounts = (raw.counts !== null && typeof raw.counts === "object" && !Array.isArray(raw.counts)
        ? raw.counts
        : {});
    const rawError = (raw.lastError !== null && typeof raw.lastError === "object" && !Array.isArray(raw.lastError)
        ? raw.lastError
        : null);
    const errorAt = rawError === null ? null : toEpoch(rawError.at);
    const lastError = rawError !== null && isErrorCode(rawError.code) && errorAt !== null
 ...
 * with the install".
 */
export function readBeaconAt(path) {
    if (!existsSync(path))
        return ok(null);
    let raw;
    try {
        ...
        return fsError(cause, path, `read ${BEACON_FILENAME}`);
    }
    let parsed;
    try {
        ...
        return err("BEACON_MALFORMED", "The status file could not be read as JSON — it may have been caught mid-write.", { path, cause });
    }
...
export function readBeacon(platform = process.platform, env = process.env, home = homedir()) {
