 ...
 */
const BACKUP_GONE_REASONS = ["our-patch-without-backup", "asar-and-backup-missing"];
...
class Run {
    ...
    decide(kind, outcome, reason, fields = {}) {
        const decision = { at: this.ports.now(), kind, outcome, reason, fields };
    ...
    async alert(code, message, detail) {
        const alert = { code, message, detail, at: this.ports.now() };
        const raised = await raiseAlert(this.state, alert, { notify: a => this.ports.notify(a), productDir: this.ports.productDir, now: () => this.ports.now() }, this.options.alertRepeatMs ?? DEFAULT_REPEAT_MS);
    ...
    clear(code) {
        ...
        if (cleared)
            this.decide("alert", `${code}:resolved`, "the condition no longer holds");
    }
}
/**
 ...
 */
export async function runHelperOnce(ports, options = {}) {
...
function readInstalledBundle(run) {
...
function decideRepatch(run, entry, bundle) {
    switch (entry.state.kind) {
        ...
        case "patched-by-us": {
            ...
            if (verified.ok)
                return { reason: "none", detail: "the patch is present and matches the installed bundle" };
...
async function reconcile(run, entry, bundle, trigger) {
    const { install } = entry;
    const versionChanged = entry.version !== null && entry.knownVersion !== null && entry.version !== entry.knownVersion;
    if (versionChanged) {
...
function rememberInstall(run, entry, buildId, patchedNow) {
 ...
 * logged either way.
 */
async function handlePatchFailure(run, entry, error, need, trigger) {
    const { install } = entry;
    ...
    if (error.code === "ROLLBACK_FAILED") {
        await run.alert("rollback-failed", "Subline could not repair Discord after it updated, and could not put Discord's own files back. "
            + "Open Subline to see what to do.", { code: error.code, path: install.rootPath });
        return;
    }
    if (brokenReason !== null && BACKUP_GONE_REASONS.includes(brokenReason)) {
        await run.alert("backup-missing", "Subline cannot repair Discord because the copy of Discord's original files is gone. "
            + "Open Subline for the fix.", { code: error.code, broken: brokenReason, path: install.rootPath });
        return;
    }
    if (IMMEDIATE_PATCH_ALERTS.includes(error.code) || failures >= REPATCH_FAILURES_BEFORE_ALERT) {
        await run.alert("repatch-failed", "Discord updated and Subline could not put its translation back. Discord itself is fine — "
            + "open Subline to finish.", { code: error.code, failures, path: install.rootPath });
    }
}
/* ------------------------------------------------------------------------ *
 * Trigger B — a new mod build
 * ------------------------------------------------------------------------ */
async function maybeUpdate(run, managed, bundle) {
    ...
    const due = (run.options.forceUpdateCheck ?? false)
        || last === null
    ...
    for (const entry of managed) {
        ...
        if (refreshed !== null)
            await reconcile(run, refreshed, installedNew, "new mod build");
    }
}
/** Re-read an install after the bundle underneath it changed. */
function refresh(run, entry) {
...
async function downloadAndInstall(run, manifest) {
...
async function failUpdate(run, error) {
    ...
    if (immediate || failures >= UPDATE_FAILURES_BEFORE_ALERT) {
        await run.alert("update-failed", immediate
            ...
            : "Subline has not been able to check for updates. If translation stops working after a Discord "
                + "update, open Subline.", { code: error.code, failures });
    }
}
/* ------------------------------------------------------------------------ *
 * Health
 * ------------------------------------------------------------------------ */
async function checkHealth(run, managed) {
    ...
    await run.alert("mod-stale", "Subline is translating but nothing is reaching Discord's screen. Discord has probably changed and "
        + "Subline needs an update that is not available yet.", { buildId: installedBuildId, observations: observation.observations });
}
function patchedAtOf(entry) {
    const parsed = entry.marker?.patchedAt === undefined ? NaN : Date.parse(entry.marker.patchedAt);
    return Number.isFinite(parsed) ? parsed : 0;
}
/* ------------------------------------------------------------------------ */
function summarize(run, managed) {
    if (managed === 0)
        return "Subline is not installed in any Discord that could be found.";
    const parts = [];
    if (run.repatched.length > 0)
        parts.push(`re-patched ${run.repatched.length}`);
    if (run.deferred.length > 0)
        parts.push(`deferred ${run.deferred.length}`);
    if (run.failed.length > 0)
        parts.push(`failed ${run.failed.length}`);
    if (run.updateInstalled !== null)
        parts.push(`installed build ${run.updateInstalled}`);
    if (run.health !== null)
        parts.push(`health ${run.health.status}`);
