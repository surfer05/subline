...
export function helperProgramArguments(appPath, executableName = "Subline") {
    return [join(appPath, "Contents", "MacOS", executableName), HELPER_FLAG];
}
export function launchAgentPlistPath(home, label = HELPER_LABEL) {
 ...
 */
export function helperLaunchAgentSpec(appPath, intervalSeconds = DEFAULT_INTERVAL_SECONDS, executableName = "Subline") {
    return {
        label: HELPER_LABEL,
...
export async function installLaunchAgent(options) {
    ...
    if (platform !== "darwin") {
        return err("HELPER_REGISTRATION_FAILED", "LaunchAgents are a macOS mechanism; Windows uses a Scheduled Task instead.");
    }
    ...
    try {
        ...
        return fsError(cause, plistPath, "write the Subline helper's LaunchAgent");
    }
    ...
    if (!bootstrapped.ok) {
        ...
        return err("HELPER_REGISTRATION_FAILED", bootstrapped.error.message, { path: plistPath });
    }
    ...
    if (!loaded) {
        return err("HELPER_REGISTRATION_FAILED", "The Subline helper was registered but macOS does not list it, so background updates would not run.", { path: plistPath });
    }
...
export async function removeLaunchAgent(options) {
    ...
    if (wasLoaded) {
        ...
        if (!booted.ok) {
            return err("HELPER_REGISTRATION_FAILED", `The Subline background helper could not be unregistered (${booted.error.message}), so its file was left in place rather than leaving a running agent with no configuration.`, { path: plistPath });
        }
    }
    ...
    if (existed) {
        try {
            ...
            return fsError(cause, plistPath, "remove the Subline helper's LaunchAgent");
        }
    }
    return ok(wasLoaded || existed);
}
/** Read a plist back — used by tests and by the app's "is the helper installed?" check. */
export function readLaunchAgentPlist(plistPath) {
    try {
