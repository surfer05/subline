...
export function emptyHelperState() {
    return {
...
export function helperStatePathFor(productDir) {
 ...
 */
export function parseHelperState(raw) {
    let parsed;
    try {
    ...
    if (isRecord(parsed.alerts)) {
        for (const [code, value] of Object.entries(parsed.alerts)) {
            ...
            if (firstAt === null || lastNotifiedAt === null)
                continue;
...
export function readHelperState(path) {
    let raw;
    try {
 ...
 */
export function writeHelperState(path, state) {
    const temp = `${path}.tmp`;
    try {
        ...
        return fsError(cause, path, `write ${HELPER_STATE_FILENAME}`);
    }
    return ok(path);
}
