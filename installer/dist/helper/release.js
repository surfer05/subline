 ...
 */
export const ALLOWED_RELEASE_HOSTS = [
    "github.com",
    "api.github.com",
 ...
 */
export function assertTrustedUrl(url, what) {
    let parsed;
    try {
        parsed = new URL(url);
    }
    catch {
        return err("RELEASE_MALFORMED", `The ${what} URL is not a URL: ${url}`);
    }
    if (parsed.protocol !== "https:") {
        return err("RELEASE_UNVERIFIED", `The ${what} URL is not https, so it will not be fetched: ${url}`);
    }
    if (!ALLOWED_RELEASE_HOSTS.includes(parsed.hostname)) {
        return err("RELEASE_UNVERIFIED", `The ${what} URL points at ${parsed.hostname}, which is not one of Subline's release hosts.`);
    }
    return ok(parsed);
}
 ...
 * nothing.
 */
export function parseReleaseManifest(text, source) {
    let parsed;
    try {
        parsed = JSON.parse(text);
    }
    catch (cause) {
        return err("RELEASE_MALFORMED", "The release feed did not return readable JSON.", {
            path: source,
            cause
        });
    }
    if (!isRecord(parsed)) {
        return err("RELEASE_MALFORMED", "The release feed did not return an object.", {
            path: source
        });
    }
    if (parsed.product !== "subline") {
        return err("RELEASE_MALFORMED", "The release manifest is not one of ours.", { path: source });
    }
    if (parsed.format !== RELEASE_MANIFEST_FORMAT) {
        return err("RELEASE_MALFORMED", `The release manifest is format ${String(parsed.format)}, and this helper understands ${RELEASE_MANIFEST_FORMAT}. Update Subline itself.`, { path: source });
    }
    if (typeof parsed.buildId !== "string" || !BUILD_ID_PATTERN.test(parsed.buildId)) {
        ...
        return err("RELEASE_MALFORMED", "The release manifest has no usable build id.", {
            path: source
        });
    }
    const artifact = parsed.artifact;
    if (!isRecord(artifact)) {
        return err("RELEASE_MALFORMED", "The release manifest names no artefact.", { path: source });
    }
    if (typeof artifact.name !== "string" || artifact.name.length === 0) {
        return err("RELEASE_MALFORMED", "The release artefact has no name.", { path: source });
    }
    if (typeof artifact.url !== "string") {
        return err("RELEASE_MALFORMED", "The release artefact has no URL.", { path: source });
    }
    if (typeof artifact.sha256 !== "string" || !SHA256_PATTERN.test(artifact.sha256)) {
        ...
        // check there is.
        return err("RELEASE_MALFORMED", "The release artefact has no SHA-256, so nothing about the download could be verified.", { path: source });
    }
    if (typeof artifact.bytes !== "number" || !Number.isSafeInteger(artifact.bytes) || artifact.bytes <= 0) {
        return err("RELEASE_MALFORMED", "The release artefact has no usable byte count.", {
            path: source
        });
    }
    ...
    const signature = isRecord(rawSignature)
    ...
    if (REQUIRE_SIGNATURE && signature === null) {
        return err("RELEASE_UNVERIFIED", "The release is not signed, and this build of Subline only installs signed releases.", { path: source });
    }
    return ok({
...
export const checksumVerifier = {
    ...
    verify({ manifest, bytes }) {
        if (bytes.byteLength !== manifest.artifact.bytes) {
            ...
            return err("RELEASE_UNVERIFIED", `${manifest.artifact.name} should be ${manifest.artifact.bytes} bytes but ${bytes.byteLength} arrived.`);
        }
        const digest = sha256Of(bytes);
        if (digest !== manifest.artifact.sha256) {
            return err("RELEASE_UNVERIFIED", `${manifest.artifact.name} does not match the checksum published with it (expected ${manifest.artifact.sha256}, got ${digest}).`);
        }
        return ok(true);
    }
};
/** The checks applied today. A signature verifier is appended here, not woven in. */
export const DEFAULT_VERIFIERS = [checksumVerifier];
export function verifyDownload(manifest, bytes, verifiers = DEFAULT_VERIFIERS) {
    if (verifiers.length === 0) {
        // Refusing an empty list is what stops "verification" from becoming a
        // no-op the day somebody passes a filtered array.
        return err("RELEASE_UNVERIFIED", "No verification was configured, so the download was not installed.");
    }
    const verifiedBy = [];
...
export function isNewerBuild(manifest, installedBuildId) {
