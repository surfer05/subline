 ...
 */
export function removePluginSettings(settingsPath) {
    ...
    try {
        ...
        if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
            return err("IO_ERROR", `Vencord's settings file at ${settingsPath} could not be read, so Subline's settings were left in place rather than risking the rest of the file.`, { path: settingsPath });
        }
        root = parsed;
    }
    catch (cause) {
        return err("IO_ERROR", `Vencord's settings file at ${settingsPath} could not be read, so Subline's settings were left in place rather than risking the rest of the file.`, { path: settingsPath, cause });
    }
    const plugins = root.plugins;
    if (typeof plugins !== "object" || plugins === null || Array.isArray(plugins))
        return ok(false);
    const table = plugins;
    if (!(PLUGIN_SETTINGS_KEY in table))
        return ok(false);
    ...
    try {
        ...
        return fsError(cause, settingsPath, "remove Subline's settings from Vencord's settings file");
    }
    return ok(true);
}
/**
 ...
 */
export function uninstall(ports, options) {
    const problems = [];
    const restores = [];
    ...
    if (!keepSettings) {
        ...
        if (ports.productDir !== null && existsSync(ports.productDir)) {
            try {
                ...
                const failure = fsError(cause, ports.productDir, "remove Subline's data folder");
                if (!failure.ok)
