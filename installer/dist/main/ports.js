...
import { defaultLanguage, discordSettingsPathFor, readDiscordLocale, setTargetLanguage, vencordSettingsPathFor } from "../app/language.js";
...
export function parseTasklistCsv(stdout) {
    ...
    for (const line of stdout.split("\n")) {
        ...
        if (name.length === 0 || !Number.isFinite(pid))
            continue;
 ...
 */
export async function requestQuit(branch, platform, exec) {
    if (platform === "win32") {
...
export async function openUrl(url, platform, exec) {
    if (platform === "win32")
        await exec("cmd", ["/c", "start", "", url]);
    else
        await exec("/usr/bin/open", [url]);
}
async function launchDiscord(install, platform, exec) {
    try {
        if (platform === "win32") {
        ...
        return err("IO_ERROR", "Subline could not start Discord for you.", {
 ...
 */
export function createFlowPorts(options) {
    const platform = options.platform ?? process.platform;
    const env = options.env ?? process.env;
    const home = options.home ?? homedir();
    const exec = options.exec ?? (async (file, args) => run(file, args));
    ...
    return {
        ...
        installModBundle: () => runtimeDir === null
...
export function uninstallPaths(platform = process.platform, env = process.env, home = homedir()) {
    return {
...
export function logDirFor(platform = process.platform, env = process.env, home = homedir()) {
    if (platform === "darwin")
        return join(home, "Library", "Logs", "Subline");
    if (platform === "win32" && env.LOCALAPPDATA)
        return join(env.LOCALAPPDATA, "Subline", "logs");
    return join(home, ".subline", "logs");
}
export function defaultLanguageFor(platform = process.platform, env = process.env, home = homedir()) {
