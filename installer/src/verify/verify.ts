...
export type VerificationStatus =
    ...
    | "not-loaded"
    /** A beacon exists, but it predates this install. It vouches for nothing. */
    | "stale-beacon"
    ...
    | "loaded-erroring"
    /** Translations are being produced but none has been painted (spec §6). */
    | "translating-not-rendering"
    /** Working. Google's ≈ line only — no ✦ upgrade has appeared. */
    | "translating-approx"
    /** Working, including the ✦ upgrade. */
    | "translating-upgraded";

export interface VerificationReport {
    status: VerificationStatus;
    /**
     * THE GREEN TICK, and nothing else may set it. True only when a subtitle
     * was painted after we launched Discord.
     */
    confirmed: boolean;
    /** The mod reported in for THIS launch. */
    loaded: boolean;
    /** It is still worth waiting and asking again. */
    pending: boolean;
    ...
    errorCode: BeaconErrorCode | null;
    /** Present whenever a beacon was read successfully, stale or not. */
    beacon: Beacon | null;
    /** Set when the beacon could not be read at all — the named error, for the log. */
...
export interface VerifyOptions {
    ...
    patchedAt: number;
    /**
     * When we launched Discord, epoch ms. The stronger reference of the two:
     * this is the launch whose plugin load we are waiting for.
     */
    launchedAt: number;
    /** Defaults to `Date.now()`. Injected so the whole module is deterministic. */
    now?: number;
    /**
     * How long after the reference instant it is still worth waiting. Discord
     * has to start, log in, resolve webpack and render a channel before the
     * plugin can say anything, and a translation needs a foreign message to
     * arrive. 90s is generous on purpose: reporting failure early is the same
     * mistake as reporting success early.
     */
    timeoutMs?: number;
    /**
     * Slack on every comparison, for clock granularity and for a plugin that
     * legitimately loads a moment "before" our own recorded launch time — the
     * two clocks are the same one here, but the file's timestamps are
     * millisecond-truncated ISO strings.
     */
    skewToleranceMs?: number;
    /** Read from here instead of the platform's standard location. */
    beaconPath?: string;
 ...
 */
function referenceInstant(options: VerifyOptions): number {
    return Math.max(options.patchedAt, options.launchedAt);
}

function report(partial: Partial<VerificationReport> & {
    status: VerificationStatus;
    summary: string;
}): VerificationReport {
    return {
 ...
 */
export function verifyOnce(options: VerifyOptions): VerificationReport {
    ...
    const read: Result<Beacon | null> = options.beaconPath !== undefined
    ...
    if (!read.ok) {
        return report({
            ...
            summary:
                "Subline is installed, but its status file could not be read, so we cannot confirm "
...
export interface AwaitVerifyOptions extends VerifyOptions {
    /** How often to re-read the beacon. */
 ...
 */
export async function awaitVerification(options: AwaitVerifyOptions): Promise<VerificationReport> {
