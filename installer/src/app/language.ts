...
export interface LanguageOption {
    /** The bare code stored in the setting. */
    code: string;
    /** The language's name IN ITSELF — "Türkçe". */
    endonym: string;
    /** The same name in the installer's UI language, for a secondary line. */
    englishName: string;
}
 ...
 */
export function normalizeLangCode(input: string | null | undefined): string | null {
 ...
 */
export function endonymOf(code: string): string | null {
 ...
 */
export function languageOptions(codes: readonly string[] = SUPPORTED_LANGUAGE_CODES): LanguageOption[] {
    const options: LanguageOption[] = [];
    for (const raw of codes) {
        const code = normalizeLangCode(raw);
        if (code === null) continue;
        if (options.some(option => option.code === code)) continue;
...
export function discordSettingsPathFor(
    platform: NodeJS.Platform = process.platform,
    env: NodeJS.ProcessEnv = process.env,
    home: string = homedir()
): string | null {
    if (platform === "darwin") return join(home, "Library", "Application Support", "discord", "settings.json");
    if (platform === "win32") {
 ...
 */
export function readDiscordLocale(settingsPath: string | null): string | null {
    if (settingsPath === null || !existsSync(settingsPath)) return null;
    try {
        const parsed: unknown = JSON.parse(readFileSync(settingsPath, "utf8"));
        if (typeof parsed !== "object" || parsed === null) return null;
        const { locale } = parsed as { locale?: unknown };
...
export function defaultLanguage(options: {
    ...
    for (const candidate of [options.discordLocale, options.systemLocale]) {
        ...
        if (isKnownLanguage(candidate)) return normalizeLangCode(candidate) as string;
    }
    return "en";
}
 ...
 */
export function vencordSettingsPathFor(
    platform: NodeJS.Platform = process.platform,
    env: NodeJS.ProcessEnv = process.env,
    home: string = homedir()
): string | null {
    if (env.VENCORD_USER_DATA_DIR) return join(env.VENCORD_USER_DATA_DIR, "settings", "settings.json");
    if (env.DISCORD_USER_DATA_DIR) return join(env.DISCORD_USER_DATA_DIR, "..", "VencordData", "settings", "settings.json");
    if (platform === "darwin") {
...
export interface SetTargetLanguageReport {
    path: string;
    code: string;
    /** What the setting was before, when there was one. */
 ...
 */
export function setTargetLanguage(settingsPath: string | null, rawCode: string): Result<SetTargetLanguageReport> {
    const code = normalizeLangCode(rawCode);
    if (code === null || !isKnownLanguage(code)) {
        return err<SetTargetLanguageReport>(
            "IO_ERROR",
            `"${rawCode}" is not a language code, so it was not saved.`
        );
    }
    if (settingsPath === null) {
        return err<SetTargetLanguageReport>(
            "IO_ERROR",
            "Could not work out where Vencord keeps its settings on this platform, so the reading language was not saved."
        );
    }
    ...
    if (!created) {
        ...
        try {
            ...
            return fsError<SetTargetLanguageReport>(cause, settingsPath, "read Vencord's settings");
        }
        try {
            ...
            return err<SetTargetLanguageReport>(
                "IO_ERROR",
                `Vencord's settings file at ${settingsPath} could not be read as JSON, so the reading language was not saved. Subline will not overwrite it.`,
    ...
    try {
        ...
        return fsError<SetTargetLanguageReport>(cause, settingsPath, "save the reading language into Vencord's settings");
    }
...
export function readTargetLanguage(settingsPath: string | null): string | null {
    if (settingsPath === null || !existsSync(settingsPath)) return null;
    try {
        const parsed: unknown = JSON.parse(readFileSync(settingsPath, "utf8"));
