...
describe("setSublineCode", () => {
    it("stores the code and selects the Subline (relay) engine that uses it", () => {
        const path = join(dir, "settings.json");
        const result = setSublineCode(path, "slp_abcdefghijklmnop");
        expect(result.ok).toBe(true);

        const written = JSON.parse(readFileSync(path, "utf8"));
        const plugin = written.plugins[PLUGIN_SETTINGS_KEY];
        expect(plugin.sublineCode).toBe("slp_abcdefghijklmnop");
        // A code with no engine selected changes nothing: the plugin would go on
        // using Google and the user would have pasted a code for no result.
        expect(plugin.engine).toBe("relay");
        expect(plugin.enabled).toBe(true);
    });

    it("trims a pasted code", () => {
        // A code arriving with a trailing newline is rejected by the relay,
        // which the plugin then surfaces as a bad code — sending somebody to
        // replace a code that was correct.
        const path = join(dir, "settings.json");
        setSublineCode(path, "  slp_padded  \n");
        const written = JSON.parse(readFileSync(path, "utf8"));
        expect(written.plugins[PLUGIN_SETTINGS_KEY].sublineCode).toBe("slp_padded");
    });

    it("refuses an empty code rather than storing one", () => {
        const path = join(dir, "settings.json");
        const result = setSublineCode(path, "   ");
        expect(result.ok).toBe(false);
        expect(existsSync(path)).toBe(false);
    });

    it("never returns the code, only its length", () => {
        const result = setSublineCode(join(dir, "settings.json"), "slp_secret");
        expect(result.ok).toBe(true);
        if (!result.ok) return;
        expect(JSON.stringify(result.value)).not.toContain("slp_secret");
        expect(result.value.codeLength).toBe("slp_secret".length);
    });
    ...
    it("refuses a settings file it cannot parse rather than replacing it", () => {
        ...
        const result = setSublineCode(path, "slp_x");
        expect(result.ok).toBe(false);
